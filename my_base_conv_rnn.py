...
"""
  ...
  def get_initial_state(self, inputs):
    ...
    initial_state = my_backend.zeros_like(inputs)
    # (samples, img_dims..., filters)
    initial_state = my_backend.sum(initial_state, axis=1)
  ...
  def call(self,
    ...
    if isinstance(mask, list):
      mask = mask[0]
    timesteps = my_backend.int_shape(inputs)[1]
    ...
    last_output, outputs, states = my_backend.rnn(step,
                                                  inputs,
                                                  initial_state,
                                                  constants=constants,
    ...
    if self.stateful:
      updates = [
        my_backend.update(self_state, state)
  ...
  def reset_states(self, states=None):
    ...
    if self.states[0] is None:
      if hasattr(self.cell.state_size, '__len__'):
        self.states = [my_backend.zeros(get_tuple_shape(dim))
                       for dim in self.cell.state_size]
      else:
        self.states = [my_backend.zeros(get_tuple_shape(self.cell.state_size))]
    elif states is None:
      if hasattr(self.cell.state_size, '__len__'):
        for state, dim in zip(self.states, self.cell.state_size):
          my_backend.set_value(state, np.zeros(get_tuple_shape(dim)))
      else:
        my_backend.set_value(self.states[0],
      ...
      for index, (value, state) in enumerate(zip(states, self.states)):
        ...
        my_backend.set_value(state, value)
